...
def period_estimator(transition_times: np.ndarray,
    ...
    # Tails of the autocorrelogram are too noisy, keep only a fraction of width around the
    # central peak
    mid_xcorr = len(xcorr) // 2
    width_xcorr = int(len(xcorr) * width_around_peak)
    peaks, _ = find_peaks(xcorr_smooth[mid_xcorr-width_xcorr:mid_xcorr+width_xcorr],
                          distance=distance_between_peaks
                         )

    # To avoid boundary effects due to the arbitrary width cut above, remove boundary peaks
